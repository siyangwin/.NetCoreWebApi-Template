...
namespace Core
{
    ...
    public class HttpHelper
    {
        ...
        //public HttpHelper(ILogService logService)
        //{
        //    //日志
        ...
        public HttpResult GetHtml(HttpItem item, HttpContext httpContext = null)
        {
            ...
            finally
            {
                if (httpContext != null)
                {
                    //string reqParameter = "URL:" + item.URL + (item.PostdataByte != null && item.PostdataByte.Length != 0 ? Encoding.UTF8.GetString(item.PostdataByte) : "");

                    //string resParameter = JsonConvert.SerializeObject(result);

                    //string time = (DateTime.Now - ReqTime).ToString();

                    //Exception ex = null ;
                    ////写入日志
                    //GlobalConfig.SystemLogService.LogAdd(SystemLogTypeEnum.Information, httpContext, "第三方接口", "", "", "",ex);
                }
            }
            return result;
        }
