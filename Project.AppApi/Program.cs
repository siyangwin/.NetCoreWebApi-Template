...
builder.Services.AddCors(options =>
{
    ...
    {
        var origins = builder.Configuration.GetValue<string>("ConfigSettings:Orgins")?.Split(',') ?? Array.Empty<string>();
        
        if (origins.Length > 0 && !origins.Contains("*"))
        {
            builders
                .WithOrigins(origins)  // 指定明确的域名
                .AllowAnyHeader()  // 所有标头
        ...
        {
            builders
                .AllowAnyOrigin()  // 所有来源（不能与 AllowCredentials 同时使用）
                .AllowAnyHeader()  // 所有标头
                .AllowAnyMethod();  // 所有 HTTP 方法
        }
    });
